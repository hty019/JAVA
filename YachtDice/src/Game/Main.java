...
public class Main {
	...
	public static void main(String[] args) throws InterruptedException, IOException {
		...
		System.out.print("1p user의 닉네임을 입력하세요.");
		p1 = br.readLine();
		System.out.print("2p user의 닉네임을 입력하세요>");
		p2 = br.readLine();
		System.out.println(p1 + "님과 " + p2 + "님의 Yacht Dice를 시작합니다.");
		for (int i = 3; i > 0; i--) {
			time();
			System.out.print(".");
		}
...
//		time();
		while (true) {
//			System.out.println(p1 + "님의 차례입니다. 주사위를 던집니다.");
//			dice = rollDiceAll(dice);
//			printRoll(dice);
//			time();
//			dice = reRollDice(dice);
//			p1inputScore(dice, sb);
//			time();
			System.out.println(p2 + "님의 차례입니다. 주사위를 던집니다.");
			dice = rollDiceAll(dice);
			printRoll(dice);
			time();
			dice = reRollDice(dice);
			p2inputScore(dice, sb);
			gameOverCheck(p1, p2, sb);
				...
				while (inputLoop) {
					...
					while (st.hasMoreTokens()) {
						...
						if (ans.equals("all")) {
							...
							for (int i = 0; i < diceIdxBox.length; i++) {
								temp[i] = diceIdxBox[i];
							}
